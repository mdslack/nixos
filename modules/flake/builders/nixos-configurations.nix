{
  ...
  mkConfig = leafName: leafModule: let
    ...
    facterModule =
      if builtins.hasAttr "facter" inputs.nixos-facter-modules.nixosModules
      then inputs.nixos-facter-modules.nixosModules.facter
      else null;
  ...
  in
    lib.nameValuePair outputName (inputs.nixpkgs.lib.nixosSystem {
